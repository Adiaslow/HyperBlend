...
class CompoundService:
    ...
    def __init__(self, session: Session):
        ...
        self.cache_duration = timedelta(days=30)  # Cache external data for 30 days
    ...
    def get_compound_by_pubchem_id(self, pubchem_id: str) -> Optional[Compound]:
        ...
        if compound:
            # Check if external data needs to be refreshed
            last_updated = getattr(compound, "last_updated", None)
            if (
                not last_updated
                or datetime.utcnow() - last_updated > self.cache_duration
            ):
        ...
        setattr(compound, "last_updated", datetime.utcnow())
